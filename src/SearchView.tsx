...
/** `#` can't ride in a query string, so the numeric bucket travels as `0-9`. */
const paramToBucket = (v: string | null): Bucket | null => {
  if (!v) return null
  const k = v === '0-9' ? '#' : v.toUpperCase()
  return (BUCKETS as readonly string[]).includes(k) ? (k as Bucket) : null
}
const bucketToParam = (b: Bucket) => (b === '#' ? '0-9' : b)
...
export default function SearchView() {
  ...
  /** Confined to one letter; otherwise the letter is just a starting point. */
  ...
  const selectBucket = (b: Bucket | null) => {
    patch({ letter: b ? bucketToParam(b) : null, q: null, mode: b ? params.get('mode') : null })
  ...
  /** Where this view starts inside the full A→9 run. */
  ...
  /** Letter runs, so a continuous list reads as A … Z … 0–9 as you scroll. */
  const groups = useMemo(() => {
    if (searching || confined) return [{ key: 'flat', label: null, items: entries }]
    const out: { key: string; label: string; items: CatalogEntry[] }[] = []
    for (const e of entries) {
      const b = bucketOf(e.name)
      const last = out[out.length - 1]
      if (last && last.key === b) last.items.push(e)
      else out.push({ key: b, label: bucketLabel(b), items: [e] })
    }
    return out
        ...
        <div className="mb-5 flex flex-wrap items-baseline justify-between gap-x-6 gap-y-3">
          <h2 className="font-sans text-[13px] text-sage-600">
            {searching
              ...
              : confined && bucket
                ? <>Drugs beginning with <span className="font-medium text-sage-900">{bucketLabel(bucket)}</span></>
                : bucket
                  ? <>The catalog from <span className="font-medium text-sage-900">{bucketLabel(bucket)}</span> onward</>
                  : 'The full catalog, A to 9'}
            {ready && total > 0 && (
            ...
            <div className="flex flex-wrap items-center gap-4">
              {bucket && !searching && (
                <div className="flex items-center rounded-lg border border-sage-200 bg-white p-0.5">
                  <ScopeButton
                    label="Browse on"
                    title={`Start at ${bucketLabel(bucket)} and keep going through 0–9`}
                    active={!confined}
                    onClick={() => patch({ mode: null })}
                  />
                  <ScopeButton
                    label={`Only ${bucketLabel(bucket)}`}
                    title={`Show just the ${bucketLabel(bucket)} entries`}
                    active={confined}
          ...
          <>
            {groups.map(g => (
              <div key={g.key} className="mb-10 last:mb-0">
                {g.label && (
                  <div id={`letter-${bucketToParam(g.key as Bucket)}`} className="mb-3 flex items-center gap-3 scroll-mt-28">
                    <span
                    ...
                    <span className="font-mono text-[11px] text-sage-400">
                      {counts ? counts[g.key as Bucket].toLocaleString() : ''}
                    </span>
                  </div>
                )}
...
/**
 * A–Z then 0–9, set as type rather than as chips: the active letter carries an
 * aqua fill, empty letters stay inert. Scrolls horizontally on narrow screens.
 */
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 overflow-x-auto sm:flex-wrap sm:overflow-visible">
        ...
        <span className="mx-1 h-4 w-px shrink-0 bg-sage-200" aria-hidden="true" />
        ...
        {BUCKETS.map(b => {
          ...
          return (
            <button
              ...
              title={n ? `${n.toLocaleString()} entries` : 'No entries'}
              className={`shrink-0 rounded px-[7px] py-1 font-mono text-[13px] leading-none transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-aqua-600 ${
                !n
