...
export async function loadCatalog(): Promise<CatalogEntry[]> {
  ...
  _loading = (async () => {
    ...
    for (;;) {
      ...
      for (const row of data as CatalogRow[]) entries.push(rowToEntry(row))
 ...
 * ~24.5k entries scans in well under a frame; no index needed client-side.
 */
