 ...
 *
 * 2026-09-25: generic combinations. Combination products (block 2) are also
 * loaded, but only to search against — they still never appear in browse. A
 * query that names two or more ingredients, in any order and with any separator
 * ("metformin sitagliptin", "metformin/sitagliptin", "sitagliptin-metformin",
 * "metformin and sitagliptin", "metformin, sitagliptin") or run together
 * ("metforminsitagliptin"), returns the combination products that contain all
 * of them, plus each named ingredient's own moiety. A single ingredient
 * ("metformin") still returns just the moiety; its combinations are on its page.
...
export async function loadCatalog(): Promise<CatalogEntry[]> {
  ...
  _loading = (async () => {
    const [rows, comboRows, comboNameRows] = await Promise.all([
      ...
      // Same for generic combination names ("sitagliptin and metformin hydrochloride").
      fetchAll<CatalogRow>((from, to) =>
        supabase
          .from('catalog_entries')
          .select('pcid,slug,name,entity_type,primary_brand,controlled_schedule,brand_names')
          .eq('entity_type', 'combination')
          .order('pcid', { ascending: true })
          .range(from, to),
      ).catch(err => {
    ...
    _comboNames = comboNameRows.map(r => {
      const entry = { ...rowToEntry(r), type: 1 as const }
...
/** Words that join ingredient names rather than name an ingredient. */
const JOINERS = new Set(['and', 'with', 'plus', 'w'])

/**
 * Salt, hydrate and release words: "metformin hydrochloride" names one
 * ingredient, not two, so these never count as an ingredient of their own.
 */
const FORM_WORDS = new Set([
  'hydrochloride', 'hcl', 'dihydrochloride', 'hydrobromide', 'sodium', 'potassium', 'calcium', 'magnesium',
  'phosphate', 'sulfate', 'maleate', 'succinate', 'tartrate', 'bitartrate', 'mesylate', 'besylate', 'acetate',
  'citrate', 'fumarate', 'bromide', 'chloride', 'monohydrate', 'dihydrate', 'trihydrate', 'anhydrous',
  'er', 'xr', 'xl', 'sr', 'cr', 'la', 'extended', 'release', 'delayed',
])

/** Pharmacy shorthand for ingredients, expanded before matching ("lisinopril/HCTZ"). */
const INGREDIENT_ABBREVIATIONS: Record<string, string> = {
...
/**
 * The ingredients a query names: "metformin/ sitagliptin", "metformin-sitagliptin",
 * "metformin and sitagliptin", "metformin, sitagliptin" all → [metformin, sitagliptin].
 */
function queryIngredients(query: string): string[] {
  return norm(query)
    .split(' ')
    .filter(t => t && !JOINERS.has(t))
    .map(t => INGREDIENT_ABBREVIATIONS[t] ?? t)
}

/**
 * Whether every query token starts a *different* word of the combination's name.
 * Longest tokens are placed first so a short token can't steal the word a
 * longer one needs. Returns how many tokens matched a whole word, or -1.
 */
function matchTokens(tokens: string[], words: string[]): number {
  const used = new Set<number>()
  let whole = 0
  for (const t of [...tokens].sort((a, b) => b.length - a.length)) {
    let hit = words.findIndex((w, i) => !used.has(i) && w === t)
    if (hit === -1) hit = words.findIndex((w, i) => !used.has(i) && w.startsWith(t))
    if (hit === -1) return -1
    if (words[hit] === t) whole++
    used.add(hit)
  }
  return whole
}
...
/**
 * Whether a run-together query ("metforminsitagliptin", "sitagliptinmetfor")
 * splits into two or more distinct words of the combination's name, in any
 * order. The last piece may be a prefix of at least three letters, so the
 * match holds while the user is still typing.
 */
function splitsInto(rest: string, words: string[], used: Set<number> = new Set()): boolean {
  if (!rest) return used.size >= 2
  for (let i = 0; i < words.length; i++) {
    if (used.has(i) || words[i].length < 3) continue
    const w = words[i]
    const next = new Set(used).add(i)
    if (rest.startsWith(w) && splitsInto(rest.slice(w.length), words, next)) return true
    if (used.size >= 1 && rest.length >= 3 && w.startsWith(rest)) return true
  }
  return false
}
...
export function searchCatalog(query: string, limit = 25): CatalogEntry[] {
  ...
  const offerCombo = (e: CatalogEntry, s: number) => {
    const prev = seenCombos.get(e.n)
    if (!prev || s > prev.s) seenCombos.set(e.n, { e, s })
  }
  for (const { brand, entry } of _comboBrands) {
    const s = scoreText(norm(brand), q, 88, 72, 52, 32)
    if (s) offerCombo({ ...entry, matchedBrand: brand }, s)
  }

  // Generic combinations named by their ingredients, in any order or spelling
  const words = queryIngredients(query)
  const tokens = words.filter(t => !FORM_WORDS.has(t))
  const bestSingle = scored.reduce((m, x) => Math.max(m, x.s), 0)
  if (words.length >= 2) {
    // Each named ingredient's own page ("metformin" for "metformin sitagliptin",
    // or for "metformin hydrochloride")
    const have = new Set(scored.map(x => x.e.n))
    for (const t of tokens) {
      const e = _bySlug?.get(t) ?? _catalog.find(c => norm(c.name) === t)
      if (e && !have.has(e.n)) {
        scored.push({ e, s: 45 })
        have.add(e.n)
      }
  ...
  if (tokens.length >= 2) {
    for (const { entry, words: comboWords } of _comboNames) {
      const whole = matchTokens(tokens, comboWords)
      if (whole < 0) continue
      // Whole-word hits rank above partly-typed ones; the name-length tie-break puts tighter combos first.
      offerCombo(entry, whole === tokens.length ? 86 : 70)
    }
  } else if (tokens.length === 1 && tokens[0].length >= 7 && bestSingle < 80) {
    const run = tokens[0]
    for (const { entry, words: comboWords } of _comboNames) {
      if (splitsInto(run, comboWords)) offerCombo(entry, 80)
    }
  }
